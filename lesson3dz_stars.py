...
def vacancy_list_forming(vacancy_list, name, link, salary_min, salary_max, site):
    ...
    '''
    if session.query(ListOfVacancy).filter(ListOfVacancy.link == link).first() is None:
        vacancy_data = {}
...
def search_vacancy_min_salary():
    ...
    for vacancy_line in result_list_vacancy:
        print(f'*****************\n'
    ...
    if flag:
        print('Слишком много просите :) Вакансий нет')
    session.close()
...
def hh_site():
    ...
    if data == '' :
        ...
        for vacancy_id in vacancy_block :
            vacancy_link = 'https://' + ''.join(regex.findall(r'(hh.ru/vacancy/[0-9]+)',
                                                               vacancy_id.find('a').get('href')))
...
def put_vacancy_bd(vacancy_list):
...
while True:
    choice = input('******************************\n'
                   'Введите 1, чтобы собрать данные в базу данных\n'
                   '        2, чтобы провести поиск по базе данных\n'
                   '        3, чтобы выйти из программы\n'
                   'Ваш выбор: ')
    if choice == '1':
        vacancy_list = []
        ...
        superjob_site()
        session.close()
        # Загрузка новых в базу
        put_vacancy_bd(vacancy_list)
    elif choice == '2':
        # Выборка из базы
        search_vacancy_min_salary()
    elif choice == '3':
        break
    else:
